...
void Timer_Setup(void)
{
  ...
  TACTL = TASSEL_2 +            // Fonte do clock: SMCLK
    TAIE +                  // Modo de contagem: Crescente, conta até atingir o TA0CCR0
      ID_1;                 // Divisão por 1
  // Bloco Capture/Compare
  TACCR0=CICLOS;                //interrupcao a cada numero de CICLOS de clocks
  TACTL &= ~TAIFG; //limpa a flag
  TACTL |= MC_1;
...
{
  if(TAIV==0x0A){ // overflow do timer e limpa flag    
